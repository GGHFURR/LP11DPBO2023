...
include_once("kontrak/kontrakformpresenter.php");

class PasienForm implements KontrakPasienFormPresenter
{
    private $tabelpasien;
    private $data = [];
    ...
    function prosesDataPasien()
    {
        try {
            ...
            while ($row = $this->tabelpasien->getResult()) {
                ...
                $pasien->setTelpon($row['telp']); //mengisi telp
    ...
    {
        $this->tabelpasien->open();
        $this->tabelpasien->UpdatePasien($id);
        $this->tabelpasien->close();
